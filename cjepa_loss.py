...
official code is public for this paper. Algorithm 1 is treated as
authoritative over the paper's looser Python-ish pseudocode elsewhere in the
same appendix, since the two aren't fully consistent.

Mechanism: reuses the M target-block predictor outputs already computed for
the main JEPA loss as "multiple views" -- no extra augmented views, no extra
encoder forward pass. For every ordered pair of target blocks (i, j), i != j,
mean-pool each block's predicted representation over patches, project
through a small shared head, and run the standard VICReg triple (sim + std
+ cov) on the pair -- averaged over all M*(M-1) ordered pairs.
...
class CJEPAProjector(nn.Module):
    """3-layer BN-ReLU MLP, same skeleton as VICReg's own projector."""
...
def cjepa_regularizer(pred_embeds, num_blocks, batch_size, projector,
    ...
    pred_embeds : (num_blocks * batch_size, N_tgt, D) -- the predictor's
                  output already computed for the main JEPA loss, block-major
                  (rows [0:B)=block 0, [B:2B)=block 1, ...). Safe here
                  because MyUtils.Patchify always returns exactly 1 context
                  mask, which makes MyUtils._repeat_interleave_batch's
                  repeat=1 call a data-order no-op on top of
                  MyUtils.apply_masks' block-major torch.cat.
    num_blocks  : M, args.num_blocks.
    batch_size  : B.
    projector   : a CJEPAProjector instance.

    Returns (loss, stats). loss is UNWEIGHTED by cjepa_weight (Eq. 21's
    outer scale) -- apply that in the training loop.
    """
